...
Consumers declare networkCore.acme.certificateClaims, ownership records and reloadServices. Native NixOS consumers register their own renewal actions: a Caddy virtual host using `useACMEHost` automatically adds `caddy.service`. Other consumers explicitly contribute `networkCore.acme.reloadServices.<certificate>`. The final merged list is deduplicated, including when Caddy is also declared explicitly. Certificates and private keys remain local under native NixOS ACME storage. Renewal preserves the current IPv4-only ACME process hardening, retry timing and DNS propagation checks; it does not toggle global host IPv6.
